...
export async function setup(gameContext) {
  ...
  ctx.api({
    ...
    addTimeInTicks: (ticks) => depositTimeInTicks(ticks),
...
function headerMainMenuDisplay() {
  ui.create(
    ...
    document
      .getElementById("header-theme")
      .getElementsByClassName("d-flex align-items-right")[0]
  );
  ui.create(
    createTimeBankDisplay(),
    document.getElementById("time-bank-display-panel")
  );
  ...
  document.getElementById("time-bank-display-button").style.fontFamily =
    "Fira Mono";
}
...
function updateTimeDisplays() {
  ...
  try {
    ...
    document.getElementById("time-bank-display-button").textContent =
      formattedTimeString;
...
function patchSidebar() {
  let showSidebar = settings
    .section("Where to show button")
    .get("show-sidebar");
  ...
  ctx.patch(Minibar, "initialize").after(() => {
    game.minibar.minibarElement.prepend(
      game.minibar.createMinibarItem("minibar-TimeBank", logo, "", {
        onClick: () =>
          document.getElementById("hover-TimeBank").classList.remove("d-none"),
      }).element
    );
  });
}
...
function additionalMinibarPatches() {
  ...
  const hover_TimeBank = document
    .getElementById("skill-footer-minibar-items-container")
    .cloneNode();
  ...
  document
    .getElementById("skill-footer-minibar-items-container")
    .parentElement.appendChild(hover_TimeBank);
    ...
    click() {
      ...
      if (DEBUG) {
        ctx.characterStorage.setItem(
          "offline_time",
          getPlayerTime() + Math.floor(Math.random() * msPerHour * 96)
        ); // Give a (0-4) days worth of time on press.
        updateTimeDisplays();
      }
    },
  };
...
function createSettings() {
  ...
  settings.section("Where to show button").add({
    ...
    onChange: (newValue, oldValue) => {
      if (newValue) {
        document
          .getElementById("time-bank-display-panel")
  ...
  settings.section("Offline Time Multiplier").add({
    ...
    label:
      "Since time skipping is 100% efficient with next to nothing wasted, it may be more realistic to only get some fraction of offline time since it is now more valuable than normal.",
    ...
    onChange: (value, previousValue) => {
      try {
        ...
        if (multiplier < 0.1 || multiplier > 10) {
          displayTimeBankToast(
            `"${multiplier}" is not a valid time multiplier [Min = 0.1, Max = 10]`,
            "danger"
          );
          return false;
        }
        ...
        displayTimeBankToast(
          `"${value}" is not a valid number [Min = 0.1, Max = 10]`,
          "danger"
        );
      }
    },
  ...
  settings.section("Maximum Offline Time").add({
    ...
    label:
      "Maximum number of offline hours that can accumulate. (Multiplier applied after max time is calculated)",
...
function simulateTime(hours) {
  ...
  if (game.activeAction === undefined) {
    displayTimeBankToast(
      `No active action, won't skip time while not training`,
      "danger"
    );
    return;
  }
  ...
  if (player_offline_time < timeToSimulate) {
    ...
    displayTimeBankToast(
      `Not enough time. Skipping ${formatTimeForDisplay(timeToSimulate)}.`,
      "info"
    );
  }
  ...
  ctx.characterStorage.setItem(
    "offline_time",
    Math.max(player_offline_time - timeToSimulate, 0)
  );
  if (DEBUG)
    console.log(
      `Successfully spent ${timeToSimulate} time [${player_offline_time} -> ${getPlayerTime()}]`
    );
...
function interceptOfflineProgress() {
  ctx.patch(Game, "processOffline").replace((originalMethod, isModCall) => {
    if (!isModCall) {
      ...
      if (
        settings.section("Maximum Offline Time").get("max-offline-time") > 0
      ) {
        // cap offline time if set
        newTime = Math.min(
          newTime,
          settings.section("Maximum Offline Time").get("max-offline-time") *
            msPerHour
        );
      }
      newTime *= settings
        .section("Offline Time Multiplier")
        .get("offline-time-multiplier");
      ...
      if (offlineTimeBank > TICK_INTERVAL) {
        displayTimeBankToast(
          `Away for ${formatTimeForDisplay(offlineTimeBank)}, time recorded`,
          "success"
        );
        updateTimeDisplays();
      }
...
function depositTimeInTicks(ticks) {
  ctx.characterStorage.setItem("offline_time", getPlayerTime() + ticks);
}

function depositTimeInHours(hours) {
    depositTimeInTicks(hours * msPerHour)
}
